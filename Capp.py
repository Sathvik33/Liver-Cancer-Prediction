...
# model = joblib.load(r'C:\C_py\Project\Cancer\Cancerpred.joblib')
# encoder = joblib.load(r'C:\C_py\Project\Cancer\encoders.joblib')
# scaler = joblib.load(r'C:\C_py\Project\Cancer\scaler.joblib')
...
# category_options = {col: encoder.categories_[i] for i, col in enumerate(categorical_columns)}

# def preprocess_input(user_inputs):
#     input_df = pd.DataFrame([user_inputs], columns=categorical_columns + numerical_columns)
...
#     numerical_df = input_df[numerical_columns].astype(float)
#     processed_input = pd.concat([numerical_df, encoded_df], axis=1)
#     return processed_input

# st.title("Liver Cancer Prediction App")
# st.write("Please select or enter the details below: ")

# user_inputs = {}
# for col in categorical_columns:
#     user_inputs[col] = st.selectbox(f"Select {col}", options=category_options[col], index=0)
...
model = joblib.load(r'C:\C_py\Project\Cancer\Cancerpred.joblib')
label_encoders = joblib.load(r'C:\C_py\Project\Cancer\encoders.joblib')
scaler = joblib.load(r'C:\C_py\Project\Cancer\scaler.joblib')
        ...
        if col in categorical_columns:
            if col in label_encoders:
                ...
                if user_inputs[col] in encoder.classes_:
                    ...
                    processed_inputs.append(-1)  # Handle unknown category
            else:
